---
title: "Species Distribution Modeling - Mola mola example"
author: "Nick Record"
date: "10/19/2020"
...
## Fetching Mola mola occurences

In the line below, I've just replaced the example species with Mola mola and renamed the target variable to M.mola.

```{r fetch_occurrences}
M.mola <- robis::occurrence("Mola mola")
```

## Plot occurrences

Plotting the Mola mola occurrences, again, I've just used the example and put in M.mola
...
ggplot2::ggplot(data = M.mola, mapping = aes(x = decimalLongitude, y = decimalLatitude)) +
  # Add occurrence data
  geom_point() +
  # Add map data
  geom_polygon(data = worldmap, aes(long, lat, group = group), fill = NA, colour = "gray43") +
        coord_quickmap(xlim = c(round(min(M.mola$decimalLongitude)), 
                                round(max(M.mola$decimalLongitude))), 
                       ylim = c(round(min(M.mola$decimalLatitude)), 
                                round(max(M.mola$decimalLatitude))),
                       expand = TRUE) +
  # Clean up theme
  theme_bw() +
...
background_locs <- as.data.frame(sst$Aug, xy = TRUE) %>% 
  ...
  dplyr::filter(!(code %in% M.mola.biomod$code) & !is.na(Aug)) %>%
  dplyr::select(lon, lat, pa, code)

# Combine occurrence data and background points
M.mola.biomod <- rbind(M.mola.biomod, background_locs)
...
ggplot2::ggplot() + 
  ...
  # Add occurrence data
  geom_point(data = M.mola.subset, mapping = aes(x = lon, y = lat), color = "blue") +
  # Add background data
...
species_occurrence <- as.numeric(unlist(M.mola.biomod[, "pa"]))
# XY coordinates of species data
species_xy <- M.mola.biomod[, c("lon","lat")]
...
list.files("M.mola/proj_current/")

# Load projection as a dataframe
proj_df <- raster::raster(file.path("M.mola/proj_current/proj_current_M.mola.grd")) %>%
  ...
  dplyr::rename(prob = M.mola_AllData_RUN1_GAM)
...
ggplot() + 
  ...
  # Add sightings data
  geom_point(data = M.mola.biomod %>% dplyr::filter(pa == 1), mapping = aes(x = lon, y = lat), color = "white", fill = "black", pch = 21, cex = 1, stroke = 0.5) +
  labs(x = "", 
       y = "", 
       title = "") +
  # Add world map data
...
The regions in yellow represent the parts of the domain where the model projects high probability of an M.mola occurrence.  The regions in black indicate that the model projected a low probability of occurrence.  This particular model doens't look too great to me.
