...
## Fetching M. novaeangliae occurences

OBIS contains global location data for marine taxa. 

Here, we will fetch humpback whale, or M. novaeangliae, occurrence data.

For more information on fetching occurrences, see 08-OBIS-access.ipynb in the 08 R tutorials folder.

```{r fetch_occurrences}
M.novaeangliae <- robis::occurrence("Megaptera novaeangliae")
```

## Plot occurrences

Next, let's plot the M. novaeangliae occurrences on a map to take a closer look at the data
...
ggplot2::ggplot(data = M.novaeangliae, mapping = aes(x = decimalLongitude, y = decimalLatitude)) +
  # Add occurrence data
  geom_point() +
  # Add map data
  geom_polygon(data = worldmap, aes(long, lat, group = group), fill = NA, colour = "gray43") +
        coord_quickmap(xlim = c(round(min(M.novaeangliae$decimalLongitude)), 
                                round(max(M.novaeangliae$decimalLongitude))), 
                       ylim = c(round(min(M.novaeangliae$decimalLatitude)), 
                                round(max(M.novaeangliae$decimalLatitude))),
                       expand = TRUE) +
  # Clean up theme
  theme_bw() +
...
Next, we will load the environmental data layers we will be using in our model.  This is done using the ohwobpg package that was loaded earlier.  Because the Gulf of Maine is an important summer feeding ground for M. novaeangliae, we will limit our environmental layer to the Gulf of Maine bounding box.  For this model, we will use sea surface temperature (sst) and chlorophyll A (chlor_a).
...
BB <- c(-75, 0, 35, 56)
nc1 <- obpg_open(urls1[1])
nc2 <- obpg_open(urls2[1])
nav1 <- obpg_nc_nav(nc1,
  bb = BB,
  res = obpg_res(what = "9km"),
  varname = "chlor_a")
nav2 <- obpg_nc_nav(nc2,
  bb = BB,
  res = obpg_res(what = "9km"),
  varname = "sst")

for (this_url in urls1){
  cat("fetching", basename(this_url), "\n")
  new_data <- obpg_fetch(this_url, nav1, outpath = path)
}

for (this_url in urls2){
  cat("fetching", basename(this_url), "\n")
  new_data <- obpg_fetch(this_url, nav2, outpath = path)
}
...
## Subset M. novaeangliae data

Next, we will subset the M. novaeangliae data so it's in the same bounding box as our environmental data.  We will also subset to only include occurrence records from August, which is the peak feeding season for M. novaeangliae in the Gulf of Maine.

```{r}
M.novaeangliae.subset <- M.novaeangliae %>% 
  # Shorten names
...
M.novaeangliae.biomod <- M.novaeangliae.subset %>%
...
background_locs <- as.data.frame(sst$Aug, xy = TRUE) %>% 
  ...
  dplyr::filter(!(code %in% M.novaeangliae.biomod$code) & !is.na(Aug)) %>%
  dplyr::select(lon, lat, pa, code)

# Combine occurrence data and background points
M.novaeangliae.biomod <- rbind(M.novaeangliae.biomod, background_locs)
...
ggplot2::ggplot() + 
  ...
  # Add occurrence data
  geom_point(data = M.novaeangliae.biomod, mapping = aes(x = lon, y = lat), color = "blue") +
  # Add background data
...
species_name <- "M.novaeangliae"
# Presence data for the species
species_occurrence <- as.numeric(unlist(M.novaeangliae.biomod[, "pa"]))
# XY coordinates of species data
species_xy <- M.novaeangliae.biomod[, c("lon","lat")]
...
Finally, we will project the model back onto the original environmental data layers.  This just means we will be using our model to make a guess, or projection, about the probability of M. novaeangliae being present in any given grid cell within our domain.
...
list.files("M.novaeangliae/proj_current/")

# Load projection as a dataframe
proj_df <- raster::raster(file.path("M.novaeangliae/proj_current/proj_current_M.novaeangliae.grd")) %>%
  ...
  dplyr::rename(prob = M.novaeangliae_AllData_RUN1_GAM)
...
ggplot2::ggplot() + 
  ...
  # Add sightings data
  geom_point(data = M.novaeangliae.biomod %>% dplyr::filter(pa == 1), mapping = aes(x = lon, y = lat), color = "white", fill = "black", pch = 21, cex = 1, stroke = 0.5) +
  labs(x = "Longitude", 
       y = "Latitude", 
       title = "August M. novaeangliae Distribution") +
  # Add world map data
  geom_polygon(data = worldmap, aes(long, lat, group = group), fill = NA, colour = "gray43") +
...
The regions in yellow represent the parts of the domain where the model projects high probability of an M. novaeangliae occurrence.  The regions in black indicate that the model projected a low probability of occurrence.  We can see that the projection lines up relatively well with the M. novaeangliae sightings data.
